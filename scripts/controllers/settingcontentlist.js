...
angular.module('lanhKdesignApp')
  .controller('SettingContentListCtrl', ["$scope", "$element", "baseService", "$filter", "$timeout", 'utilsService',
      function ($scope, $element, baseService, $filter, $timeout, utilsService) {
          ...
          $scope.changeCategory = function () {
              data.selectedAll = false;/*切换分类全选去掉*/
              loadCategoryData($scope.data.category);
          }
          ...
          var getSelectedCategoryData = function (selectAliases, callback) {
              if (selectAliases === "") {
                  ...
                  if ($scope.data.category !== 'product') {
                      condition.length = $scope.data.titleLength || 5;
                      condition.top = $scope.data.isPaged == 'false' ? 9999999 : $scope.data.pageSize;
                      condition.size = $scope.data.isPaged == 'false' ? 9999999 : $scope.data.pageSize;
                  }
          ...
          $scope.btnOK = function () {
              ...
              getSelectedCategoryData(selectedCategoies.aliases, function () {
                  if ($scope.data.row && $scope.data.col && $scope.data.category === 'product') {
                  ...
                  } else if ($scope.data.category !== 'product') {
                      if ($scope.data.$item$.length > $scope.data.pageSize && $scope.data.isPaged === 'true') {
                          $scope.data.$item$.length = $scope.data.pageSize;
                      }
                  }
                  ...
                  if ($scope.data.category === 'product') {
                      ...
                      if ($scope.data.isPaged === "true") {/*分页*/
                          dataConfig = "<order>B_Common_Item.isnew desc, B_Common_Item.id desc</order><channel>" + selectedCategoies.aliases + "</channel><count>" + $scope.data.pageSize + "</count><module>" + $scope.data.category + "</module><title>" + $scope.data.titleLength + "</title>";
                      } else if ($scope.data.isPaged === "false") {/*不分页*/
                          dataConfig = "<order>B_Common_Item.isnew desc, B_Common_Item.id desc</order><channel>" + selectedCategoies.aliases + "</channel><top>9999999</top><module>" + $scope.data.category + "</module><title>" + $scope.data.titleLength + "</title>";
                      }
                  }
                 ...
                 // $scope.data.selectedCategoies = selectedCategoies;
